...
export function convert(content: string, ignore_variables?: boolean): string {
  return content.replace(
      /\{__(name|email|author|link|date|delete|camelCaseName|pascalCaseName|snakeCaseName|kebabCaseName|lowerDotCaseName|upperCaseName)__\.?([^{}]*)\}|.*\{__omitLine__\}.*/g,
      (_, key, description) =>
          (!ignore_variables ? env.fields[key] || '' : description),
  );
}
...
export async function checkTemplatesFolder() {
  ...
  if (!await fs.exists(templatesFolderPath)) {
    await copyFolder(
        path.join(env.context.extensionPath, 'templates'), templatesFolderPath);
